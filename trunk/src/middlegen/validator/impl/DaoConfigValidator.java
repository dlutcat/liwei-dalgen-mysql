	...
	public List<ErrorMessage> validateAfterGenerate(File generatedFile, File replacedFile, FileProducer fileProcucer) {
		...
		if(replacedResources!=null && replacedResources.size()>0)
		{
			if(generatedResources==null || generatedResources.size()<=0)
			{
				...
				message.setMessage("模板文件:"+temFileName+"生成的bean定义为0条，原文件:"+replacedFileName+"的bean定义为"+replacedResources.size()+"条，请检查模板文件定义是否正确。");
				result.add(message);
				return result;
			}
			...
			for(DaoConfig resource : replacedResources)
			{
				...
				{
					ErrorMessage message=new ErrorMessage();
					message.setMessage(resource+"在:"+replacedFileName+"中有定义,在模板文件："+temFileName+"中却没有定义,"+"你可能忘记了在模板文件中定义该bean。");
					result.add(message);
				}
			}
		}
