...
public class IWalletSelect extends IWalletOperation {
	/** logger */
	public static final Logger logger = Logger.getLogger(IWalletSelect.class);
	...
	public static final String DEFAULT_MANY_RETURN_TYPE_PAGING = "com.zhz.project.common.util.page.PageList";
	...
	protected String getReturnTypeOne() {
		if (opConfig.getResultMap() != null) {
			return ((IWalletTable) getTable()).getResultMap(
					opConfig.getResultMap()).getClassAttr();
		...
		if (parser.isSelectItemSingle()) {
			...
			if (item.getAggregate() != null) {
				...
				if (aggregateFunc.equalsIgnoreCase("COUNT")) {
					...
					if (logger.isDebugEnabled()) {
						// logger.debug("The column to be aggregated is " +
						// columnName + ".");
					}

					return ((IWalletColumn) (getTable().getColumn(columnName)))
							.getJavaType();
				} else {
					// can not happen
					return "void";
				...
				return ((IWalletColumn) (getTable().getColumn(item.getColumn())))
						.getJavaType();
			}
		} else {
	...
	private void addImprotForGenericType() {
		...
		String itemType = this.getColumnType();
		if (StringUtil.equals(itemType, "long")
				|| StringUtil.equals(itemType, "int")) {
	...
	public String getMappedStatementResult() {
		if (opConfig.getResultMap() != null) {
			...
			if (((IWalletTable) getTable()).getQualifiedDOClassName().equals(
					result)) {
				return "resultMap=\""
						+ ((IWalletTable) getTable()).getResultMapId() + "\"";
	...
	public String getMappedStatementSqlForPaging() {
		...
		if (isHasSqlmap()) {
			pagingSql
					.append("select /*" + getMappedStatementId(true)
							+ "*/ * from  ")
					.append(getMappedStatementSqlNoAnnotation())
					.append(" limit #").append(getStartRowName()).append("#")
					.append(" , #").append(getItemsPerPage()).append("#");
		} else {
			pagingSql
					.append("select /*" + getMappedStatementId(true)
							+ "*/ * from  ")
	...
	public String getMappedStatementSqlForCount() {
		if (isHasSqlmap()) {
			 ...
			 */
			int indexSelectStart = StringUtil.indexOfAny(origMs, new String[] {
					"select ", "SELECT " });
			int indexSelectEnd = StringUtil.indexOfAny(origMs, new String[] {
					"from ", "FROM " });
			/*
			 * 如果没有设置countSqlEndAt属性，则默认去掉order by字句
			 * 否则，则去掉countSqlEndAt中指定字符串开头的字句 这里会将"起始字符串"到"]]>"的字符串都去掉
			 */
			int indexStart = 0;
			int indexEnd = 0;
			String endingStr = opConfig.getCountSqlEndAt();
			if (!StringUtil.isBlank(endingStr)) {
				indexStart = StringUtil.indexOfAny(origMs, new String[] {
						endingStr, StringUtil.toUpperCase(endingStr) });
			} else {
				indexStart = StringUtil.indexOfAny(origMs, new String[] {
						"order by ", "ORDER BY " });
			}
	...
	public String addSqlAnnotationForCount(String orgSql) {
		...
		String afterOperation = StringUtil.substring(orgSql,
				startOperation + 7, orgSql.length());
		orgSql = "select " + idAnnotation + " count(*) from " + operation
				+ afterOperation;
		return orgSql;
	}
