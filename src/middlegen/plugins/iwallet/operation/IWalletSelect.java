...
public class IWalletSelect extends IWalletOperation {
    ...
    public String getMappedStatementSqlForPaging() {
        ...
        if (isHasSqlmap()) {
            pagingSql.append("select /*" + getMappedStatementId(true) + "*/ * from  ").append(
                getMappedStatementSqlNoAnnotation()).append(" limit #").append(getStartRowName())
                .append("#").append(" , #").append(getItemsPerPage()).append("#");
        } else {
            pagingSql.append("select /*" + getMappedStatementId(true) + "*/ * from  ").append(
                getMappedStatementSqlNoAnnotation()).append(" limit #").append(getStartRowName())
                .append("#").append(" , #").append(getItemsPerPage()).append("#");
    ...
    public String getMappedStatementSqlForCount() {
        if (isHasSqlmap()) {
            ...
            if (!StringUtil.isBlank(endingStr)) {
                indexStart = StringUtil.indexOfAny(origMs, new String[] { endingStr,
                        StringUtil.toUpperCase(endingStr) });
            } else {
                indexStart = StringUtil.indexOfAny(origMs,
                    new String[] { "order by ", "ORDER BY " });
            }
    ...
    public String addSqlAnnotationForCount(String orgSql) {
        ...
        orgSql = "select " + idAnnotation + " count(*) from " + orgSql;
        return orgSql;
    }
