...
public class DalUtil {
    ...
    public static boolean inTabs(String tableName) throws IWalletConfigException {
        String tabs = MiddlegenTask.getTabs();
        RunModelEnum runModel = MiddlegenTask.getRunModel();
        
        if (tabs == null || "*".equals(tabs)) {
            return true;
        }
        String[] tables = tabs.split(",");

        switch (runModel) {
            case COMPARE:
                String mapName = removeTablePrefix(tableName);
                mapName = dbNameToVariableName(mapName) + "-sqlmap-mapping.xml";

                for (int i = 0; i < tables.length; i++) {
                    if (mapName.equalsIgnoreCase(tables[i].trim())) {
                        return true;
                    }
                }
            ...
            case ORGI:
                for (int i = 0; i < tables.length; i++) {
                    if (tableName.equals(tables[i].trim())) {
                        return true;
                    }
                }
        }
    ...
    public static String removeTablePrefix(String tableName) throws IWalletConfigException {
        ...
        for (String prefix : tablePrefixList) {
            if (tableName.toLowerCase().startsWith(prefix)) {
                ...
                if (MiddlegenTask.getRunModel() == RunModelEnum.COMPARE) {
                    startIndex++;
                }
                tableName = tableName.substring(startIndex);
                break;
            }
        }
        return tableName;
    ...
    public static String toUpperCaseWithDash(String name) {
        ...
        if (name == null) {
            ...
            newName = StringUtil.toUpperCaseWithUnderscores(name);
